            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content ready, section may still be generating) -->
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show special hint for final answer -->
...
const toggleRawResult = (timestamp, event) => {
  ...
  // After DOM update, adjust scroll to keep button position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract stats (e.g., "Relevant Facts: X")
    const factMatch = text.match(/Relevant Facts:\s*(\d+)/)
    ...
    // Extract sub-questions (no hard limit)
    ...
    // Extract key facts (no hard limit)
    ...
    // Extract core entities (include summary and related facts count)
    ...
    // Extract relation chain (no hard limit)
    const relSection = text.match(/### \\[Relation Chain\\]\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract active facts (no hard limit)
    ...
    // Extract historical/expired facts (no hard limit)
    ...
    // Extract entities involved (no hard limit)
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Extract interview count (e.g., "5 / 9 Agents")
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (name) {
          ...
          // Continuation of reason (exclude ending summary)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        if (questions.length > 0) {
          // If first question starts with "1.", handle separately
      ...
      // Extract answers - split Twitter/Reddit
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*Key Quotes|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Split Twitter and Reddit answers
        const twitterMatch = answerText.match(/【Twitter Answer】\n?([\s\S]*?)(?=【Reddit Answer】|$)/)
        const redditMatch = answerText.match(/【Reddit Answer】\n?([\s\S]*?)$/)
        ...
        // Fallback when only one platform marker exists
        if (!twitterMatch && redditMatch) {
          // Only Reddit answer; copy if not placeholder
          if (interview.redditAnswer && interview.redditAnswer !== '(No response on this platform)') {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer !== '(No response on this platform)') {
            interview.redditAnswer = interview.twitterAnswer
          }
        } else if (!twitterMatch && !redditMatch) {
          // No platform markers (legacy format); use full text
          interview.twitterAnswer = answerText
        }
      }
      
      // Extract key quotes (various quote formats)
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts (no hard limit)
    ...
    // Extract related edges if present
    ...
    // Extract related nodes if present
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Relation Chain (${props.result.relations.length})`)
        ]),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-questions from drift queries'),
        ...
        activeTab.value === 'relations' && props.result.relations.length === 0 && h('div', { class: 'empty-state' }, 'No relation chain yet')
      ])
    ])
  }
}
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Maintain per-question platform selection state
    const platformTabs = reactive({}) // { 'agentIdx-qIdx': 'twitter' | 'reddit' }
    
    // Get platform selection for a question
    ...
    // Check for platform placeholder text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response on this platform)' || t === '[No response]' || t === '(No response)'
    }

    // Split answers by question numbering
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Supported formats:
      // 1. "Question X:" (new format)
      // 2. "1. " or "\n1. " (legacy numeric format)
      let matches = []
      let match
      ...
      // If no numbering or only one, return whole text
      if (matches.length <= 1) {
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Split failed: return full answer for first, empty for others
      return qIdx === 0 ? answer : ''
    }
    
    // Check for dual-platform answers (excluding placeholders)
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Both platforms have real answers and differ
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual-platform switch (only when both answers exist)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading H2 (## xxx) because section title is already shown
  ...
  // Handle lists (supports sublists)
  ...
  // Handle newlines: empty lines -> paragraphs, single newline -> <br>
  html = html.replace(/\n\n/g, '</p><p class="md-p">')
  html = html.replace(/\n/g, '<br>')
  ...
  // Remove <p><br> before block elements (extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Remove consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Remove <br> before paragraph start tags after block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered list numbering
...
const getLogLevelClass = (log) => {
  ...
  // INFO uses default color (not success)
  return ''
}
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic runs after the loop on nextTick
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If task completed, scroll to top; otherwise follow latest logs
            if (isComplete.value) {
...
// Extract final answer content from the LLM response
const extractFinalContent = (response) => {
  ...
  // Try extracting content after "Final Answer:" (multiple formats)
  ...
  // If content is long and markdown-like, remove thought process first
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove leading Thought: sections
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* Dark gray, unchanged by status */
  font-weight: 500;
}
