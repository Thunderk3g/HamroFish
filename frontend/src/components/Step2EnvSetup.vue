    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create a simulation instance and fetch the world parameter template
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Using context, automatically call tools to extract entities and relations from the knowledge graph, initialize individuals, and assign behaviors and memories from reality seeds.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Related Topics Count</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM uses the simulation requirement and reality seeds to set world time flow, recommendation algorithms, active hours, posting frequency, and event triggers.
          </p>
            ...
            <div class="config-block">
              ...
              <div class="platforms-grid">
                ...
                <div v-if="simulationConfig.reddit_config" class="platform-card">
                  <div class="platform-card-header">
                    <span class="platform-name">Platform 2: Topics / Communities</span>
                  </div>
                  <div class="platform-params">
                    <div class="param-row">
                      <span class="param-label">Recency Weight</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Based on the narrative direction, automatically generate initial activation events and hot topics to seed the simulation.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Round configuration - only show after config is generated and rounds are computed -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Rounds</span>
                <span class="section-desc">ShibaSwarm auto-plans <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of real time, with each round representing <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>At 100 agents: estimated ~{{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        At 100 agents: estimated {{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For a first run, we strongly recommend switching to Custom mode to reduce rounds for a faster preview and lower error risk ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                ...
                <span class="dim-desc">Learned experience and style preferences</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Distinct Memory Imprints</span>
...
// Log de-duplication: track last key messages
let lastLoggedMessage = ''
let lastLoggedProfileCount = 0
let lastLoggedConfigStage = ''

// Rounds configuration
const useCustomRounds = ref(false) // Default to auto-generated rounds
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Enter config generation stage and start polling
    if (!configTimer) {
...
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null when config is not ready
  }
  const totalHours = simulationConfig.value.time_config.total_simulation_hours
  const minutesPerRound = simulationConfig.value.time_config.minutes_per_round
  if (!totalHours || !minutesPerRound) {
    return null // Return null when config is incomplete
  }
  const calculatedRounds = Math.floor((totalHours * 60) / minutesPerRound)
  // Ensure rounds >= 40 (recommended) to keep slider range valid
...
// Count total related topics across personas
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    // User keeps auto-generated rounds; do not pass max_rounds
    addLog(`Starting simulation, using auto rounds: ${autoGeneratedRounds.value}`)
  }
...
const startPrepareSimulation = async () => {
  ...
  // Mark step 1 complete and start step 2
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Set expected total agents immediately (from prepare response)
...
const pollPrepareStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Parse stage info and log details
      if (data.progress_detail) {
        currentStage.value = data.progress_detail.current_stage || ''
        
        // Log detailed progress (avoid duplicates)
        const detail = data.progress_detail
        const logKey = `${detail.current_stage}-${detail.current_item}-${detail.total_items}`
        if (logKey !== lastLoggedMessage && detail.item_description) {
          lastLoggedMessage = logKey
          const stageInfo = `[${detail.stage_index}/${detail.total_stages}]`
          const stageLabel = stageLabels[detail.current_stage] || detail.current_stage || 'Stage'
          if (detail.total_items > 0) {
            addLog(`${stageInfo} ${stageLabel}: ${detail.current_item}/${detail.total_items} - ${detail.item_description}`)
          } else {
            addLog(`${stageInfo} ${stageLabel}: ${detail.item_description}`)
          }
        }
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only update when API returns a valid value to avoid overwriting
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Agent personas ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || 'Unknown profession'})`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup complete. You can start the simulation.')
