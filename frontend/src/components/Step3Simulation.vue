...
const actionIds = ref(new Set()) // Action IDs for de-dup
...
const doStartSimulation = async () => {
  ...
  // Reset state to avoid interference from previous run
  resetAllState()
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`Set max rounds: ${props.maxRounds}`)
    }
...
// Track previous rounds per platform to detect changes
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Check if simulation completed (runner_status or platform completion)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'
      
      // Extra check: runner_status may lag; use platform completion flags
...
const checkPlatformsCompleted = (data) => {
  ...
  // If at least one platform completed, verify all enabled platforms
  // Use actions_count to detect enabled platforms
  ...
  // Check all enabled platforms completed
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Incrementally add new actions (de-dup)
      ...
      // Do not auto-scroll; let users browse freely
