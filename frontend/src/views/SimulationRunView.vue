...
// Read maxRounds from query on init so the child can use it immediately
...
const handleGoBack = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Environment not running; check if we should stop the process
  ...
  try {
    ...
    if (simRes.success && simRes.data) {
      ...
      try {
        ...
        addLog(`Failed to load time config, using default: ${minutesPerRound.value} minutes/round`)
      }
...
const loadGraph = async (graphId) => {
  // While simulating, avoid full-screen loading to prevent flicker
...
onMounted(() => {
  ...
  // Record maxRounds (read from query on init)
  if (maxRounds.value) {
