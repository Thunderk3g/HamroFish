  ...
  <div class="main-view">
    ...
    <header class="app-header">
      <div class="header-left">
        <div class="brand" @click="router.push('/')">ShibaSwarm</div>
      </div>
...
const handleNextStep = (params = {}) => {
  ...
  // Record max rounds configuration
  if (params.maxRounds) {
  ...
  // If custom max rounds is set, pass it via query
  if (params.maxRounds) {
 ...
 * When the user returns from Step 3 to Step 2, default to exiting the simulation
 */
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      // Try to close the simulation environment gracefully
      try {
      ...
      // Environment not running, but process might be alive; check status
    ...
    // Failure to check env status should not block the flow
...
onMounted(async () => {
  ...
  // Check and stop a running simulation (when returning from Step 3)
