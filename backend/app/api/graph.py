...
Uses project context mechanism, server-side persistence
"""
...
# Get logger
logger = get_logger('ShibaSwarm.api')
...
def build_graph():
    ...
    try:
        ...
        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": "Graph is currently building, please do not resubmit. To force rebuild, add force: true",
        ...
        # Get configuration
        graph_name = data.get('graph_name', project.name or 'ShibaSwarm Graph')
        ...
        logger.info(f"Created graph construction task: task_id={task_id}, project_id={project_id}")
        ...
        def build_task():
            build_logger = get_logger('ShibaSwarm.build')
            try:
                ...
                # Create graph construction service
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": "Graph construction task started, please query progress via /task/{task_id}"
            }
        })
