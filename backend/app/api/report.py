...
logger = get_logger('ShibaSwarm.api.report')
        ...
        def run_generate():
            try:
                ...
                if report.status == ReportStatus.COMPLETED:
                    ...
                    task_manager.fail_task(task_id, report.error or "Failed to generate report")
...
def chat_with_report_agent():
    ...
    Report Agent can autonomously call retrieval tools to answer questions in dialogue
    ...
    try:
        ...
        # Create Agent and start dialogue
        agent = ReportAgent(
            graph_id=graph_id,
            simulation_id=simulation_id,
            simulation_requirement=simulation_requirement
...
# ============== Report Progress and Sectional Interface ==============
...
def get_report_sections(report_id: str):
    """
    Get list of generated sections (Sectional output)
    
    Frontend can poll this interface to get generated section contents without waiting for the entire report to complete.
...
def check_report_status(simulation_id: str):
    ...
    Used for frontend to decide whether to unlock Interview feature
    ...
    try:
        ...
        # Only unlock interview when report is completed
...
def get_agent_log(report_id: str):
    ...
    Retrieve every action during report generation in real-time, including:
...
def stream_agent_log(report_id: str):
    """
    Get complete Agent log (One-time fetch for all)
...
def get_console_log(report_id: str):
    ...
    Unlike the structured JSON logs returned by the agent-log interface,
    this is console-style log in plain text format.
...
def stream_console_log(report_id: str):
    """
    Get complete console logs (One-time fetch for all)
