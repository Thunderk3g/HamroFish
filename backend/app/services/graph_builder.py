...
class GraphBuilderService:
    """
    Graph building service.
    Responsible for calling Zep APIs to build a knowledge graph.
    """
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        ...
        # This is required by the Zep SDK and is safe to ignore
    ...
    def add_text_batches(
        ...
        """Add text to the graph in batches. Returns a list of episode UUIDs."""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        """Wait for all episodes to finish processing (poll processed status)."""
        ...
        while pending_episodes:
            ...
            # Check processed status for each episode
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        """
        Get full graph data (including detailed info).

        Args:
            graph_id: Graph ID

        Returns:
            Dict with nodes/edges and detailed fields
        """
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)

        # Map node UUID to name
        node_map = {}
        for node in nodes:
