...
Converts entities in Zep graph to Agent Profile format required by the OASIS simulation platform

Optimizations & Improvements:
1. Calls Zep retrieval function to secondary enrich node information
2. Optimizes prompt to generate very detailed personas
...
logger = get_logger('ShibaSwarm.oasis_profile')
...
class OasisProfileGenerator:
    ...
    Converts entities in Zep graph to Agent Profiles required for OASIS simulation
    ...
    # Individual type entities (require generating detailed persona)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/Institutional type entities (require generating representative persona)
    GROUP_ENTITY_TYPES = [
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Use Zep graph hybrid search function to get rich information related to the entity
        
        Zep doesn't have a built-in hybrid search interface, so we need to search edges and nodes separately and merge the results.
        Use parallel requests to search simultaneously to improve efficiency.
        
        Args:
            entity: Entity node object
            
        Returns:
            A dictionary containing facts, node_summaries, context
        """
        ...
        try:
            ...
            logger.info(f"Zep hybrid retrieval complete: {entity_name}, obtained {len(results['facts'])} facts, {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        3. Rich information obtained through Zep hybrid retrieval
        """
        context_parts = []
        ...
        # 3. Add detailed info of associated nodes
        ...
        # 4. Use Zep hybrid retrieval to get richer info
        zep_results = self._search_zep_for_entity(entity)
        
        if zep_results.get("facts"):
            # Deduplication: exclude already existing facts
            new_facts = [f for f in zep_results["facts"] if f not in existing_facts]
            if new_facts:
                context_parts.append("### Factual Information Retrieved from Zep\n" + "\n".join(f"- {f}" for f in new_facts[:15]))
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Check if it's an individual type entity"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Check if it's a group/institutional type entity"""
    ...
    def _generate_profile_with_llm(
        ...
        - Group/Institutional entity: Generate representative account setting
        """
        ...
        # Try multiple generations until successful or maximum retry attempts reached
        max_attempts = 3
        last_error = None
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # No max_tokens set, let LLM perform freely
                )
        ...
        logger.warning(f"Failed to generate persona with LLM ({max_attempts} attempts): {last_error}, using rule-based generation")
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Check if there's an unclosed string
        # Simple check: if there's no comma or closing brace after the last quote, the string might be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First attempt to fix if truncated
        content = self._fix_truncated_json(content)
        ...
        if json_match:
            ...
            def fix_string_newlines(match):
                s = match.group(0)
                # Replace actual newlines within string with space
                s = s.replace('\n', ' ').replace('\r', ' ')
            ...
            try:
                ...
                # 5. If still failed, attempt more aggressive fix
                try:
        ...
        # 6. Attempt to extract partial info from content
        ...
        if bio_match or persona_match:
            logger.info(f"Extracted partial info from broken JSON")
            return {
                "bio": bio,
                "persona": persona,
                "_fixed": True
        ...
        logger.warning(f"JSON fix failed, returning base structure")
        return {
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in social media user persona generation. Generate detailed and realistic personas for public opinion simulation, restoring existing real-world situations to the maximum extent. Must return valid JSON format, all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
...
- 机构账号发言要符合其身份定位"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: Real-time output file path (if provided, write once per generation)
        ...
        completed_count = [0]  # Use list to modify inside closure
        lock = Lock()
        ...
        def save_profiles_realtime():
            ...
            with lock:
                ...
                try:
                    ...
                    logger.warning(f"Real-time saving profiles failed: {e}")
        
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            """Work function for generating a single profile"""
            entity_type = entity.get_entity_type() or "Entity"
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Real-time writing to file (even for fallback persona)
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Real-time output generated persona to console (complete content, no truncation)"""
        separator = "-" * 70
        ...
        # Only output to console (avoid duplicates, logger no longer outputs full content)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save Profile to file (select correct format according to platform)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile in CSV format (complying with OASIS official requirements)
        ...
        - user_char: internal use, LLM system prompt, determining how Agent thinks and acts
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        logger.info(f"Saved {len(profiles)} Reddit Profiles to {file_path} (JSON format, contains user_id field)")
