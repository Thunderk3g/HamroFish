...
Uses LangChain + Zep to implement ReACT mode for simulated report generation
...
4. Support dialogue with users, autonomously calling retrieval tools in conversation
"""
...
logger = get_logger('ShibaSwarm.report_agent')


class ReportLogger:
    """
    Detailed Log Recorder for Report Agent
    
    Generates agent_log.jsonl in the report folder, recording every step.
    Each line is a complete JSON object containing a timestamp, action type, details, etc.
    """
    ...
    def log_section_content(
        ...
        """Log section content generation complete (only logs content, doesn't represent entire section completion)"""
        self.log(
...
class ReportConsoleLogger:
    ...
    These logs are in plain text console output format, unlike agent_log.jsonl.
    """
    ...
    def _setup_file_handler(self):
        ...
        loggers_to_attach = [
            'ShibaSwarm.report_agent',
            'ShibaSwarm.zep_tools',
        ]
...
1. Automatically decompose your problem into multiple sub-questions
...
[Use Cases]
- Need for in-depth analysis of a topic
- Need to understand multiple aspects of an event
- Need to obtain rich materials to support report sections

[Return Content]
- Relevant facts (can be directly cited)
- Core entity insights
- Relationship chain analysis"""

TOOL_DESC_PANORAMA_SEARCH = """\
[Wide Search - Get Panoramic View]
This tool is used to obtain a full view of the simulation results, especially suitable for understanding the evolution process of events. It will:
...
[Use Cases]
- Need to understand the full development context of events
...
[Use Cases]
- Need to quickly find a specific information point
...
This is not LLM simulation, but a real interview interface call to get the original responses from simulation Agents.
By default, it interviews on both Twitter and Reddit platforms simultaneously to obtain more comprehensive views.
...
[Use Cases]
- Need to understand event views from different role perspectives (What do students think? What does the media think? What do officials say?)
- Need to collect opinions and positions from multiple parties
- Need to obtain real responses from simulation Agents (from OASIS environment)
- Want the report to be more vivid, including "Interview Transcript"

[Return Content]
- Identity information of interviewed Agents
- Interview responses of each Agent on both Twitter and Reddit
- Key quotes (can be directly cited)
- Interview summary and perspective comparison

[Important] Needs OASIS simulation environment running to use this feature!"""

# ── Outline Planning Prompt ──

PLAN_SYSTEM_PROMPT = """\
You are an expert in writing "Future Prediction Reports", possessing a "God's-eye view" of the simulated world—you can insight into the behaviors, remarks, and interactions of every Agent in the simulation.

[Core Concept]
We have built a simulated world and injected specific "Simulation Requirements" as variables into it. The evolutionary result of the simulated world is a prediction of what might happen in the future. What you are observing is not "experimental data", but a "rehearsal of the future".

[Your Task]
Write a "Future Prediction Reports" to answer:
...
- ✅ Focus on prediction results: event development, group reactions, emergent phenomena, potential risks
...
- Content should be concise, focusing on core prediction discoveries
...
{
    "title": "Report Title",
    "summary": "Report Summary (one sentence summarizing core prediction discovery)",
    "sections": [
        {
            "title": "Section Title",
...
Note: sections array must have minimum 2 and maximum 5 elements!"""
...
- Number of entities involved in simulation: {total_nodes}
...
2. How did various types of crowds (Agents) react and act?
...
[Reminder Again] Number of report sections: minimum 2, maximum 5, content should be concisely focused on core prediction discoveries."""

# ── Section Generation Prompt ──

SECTION_SYSTEM_PROMPT_TEMPLATE = """\
You are an expert in writing "Future Prediction Reports" and are writing a section of a report.
...
and the behaviors and interactions of Agents in the simulation are predictions of future crowd behavior.

Your task is:
- Reveal what happened in the future under the set conditions
- Predict how various types of crowds (Agents) reacted and acted
...
1. [Must call tools to observe the simulated world]
   ...
   - All content must come from events and Agent actions/words in the simulated world
   - Do not use your own knowledge to write report content
   - Each section must call tools at least 3 times (max 5 times) to observe the simulated world

2. [Must quote Agent's original words and deeds]
   - Agent speeches and behaviors are predictions of future crowd behavior
   - Use quote format (>) to display these predictions in the report:
     > "Certain group will express: original content..."
   - These citations are core evidence of the simulation prediction

3. [Language Consistency - Quote content must be translated into report language]
   - Content returned by tools may contain English or mixed Chinese-English
   - If the report language is English, ensure all quoted content is translated to fluent English
   - Keep the original meaning unchanged during translation, ensuring natural expression
   - This rule applies to both body text and blockquotes (> format)

4. [Faithfully present prediction results]
   - Report content must reflect simulated results representing the future
...
- ❌ Forbidden to use any Markdown headings (#, ##, ###, ####, etc.) within the section
- ❌ Forbidden to add the main section title at the beginning
- ✅ Section titles are automatically added by the system; you only need to write pure body content
- ✅ Use **bold**, paragraph separators, quotes, and lists to organize content, but no headings

[Correct Example]
```
This section analyzes the trend of public opinion transmission of the event. Through in-depth analysis of simulation data, we found...

**Initial Trigger Stage**

Twitter, as the first scene of public opinion, assumed the core function of initial information release:

> "Twitter contributed 68% of the initial voice volume..."

**Emotion Amplification Stage**

TikTok platform further amplified the event's influence:
...
- panorama_search: Wide-angle panoramic search, understand the full event context, timeline, and evolution process
- quick_search: Quickly verify a specific information point
- interview_agents: Interview simulated Agents to get first-person views and real reactions from different roles

═══════════════════════════════════════════════════════════════
[Workflow]
═══════════════════════════════════════════════════════════════

You can only do one of the following two things per response (cannot do both):
...
The system will execute the tool and return the result to you. You do not need to and cannot write tool returns yourself.
...
- Forbidden to fabricate tool results (Observation); all tool results are injected by the system
...
3. Use Markdown format (but forbid headings):
   ...
   - ❌ Forbidden to use any heading syntax such as #, ##, ###, ####
...
7. [Accentuate Again] Do not add any headings! Use **bold** instead of subsection titles"""
...
[Important Reminders]
1. Read the completed sections above carefully to avoid repeating content!
...
- ❌ Do not write "{section_title}" as a beginning
- ✅ Section titles are added automatically by the system
- ✅ Write body text directly; use **bold** instead of subheadings

Please begin:
1. Thought: What info is needed for this section
2. Action: Call a tool to get simulation data
3. Final Answer: Output the section content (pure body, no headings) after collecting enough info"""
...
- If info is sufficient: Output section content starting with "Final Answer:" (must quote original text above)
- If more info is needed: Call a tool to continue retrieval
═══════════════════════════════════════════════════════════════"""

REACT_INSUFFICIENT_TOOLS_MSG = (
    "[Note] You have only called {tool_calls_count} tools, at least {min_tool_calls} are required. "
    "Please call tools again to get more simulation data before outputting Final Answer. {unused_hint}"
)

REACT_INSUFFICIENT_TOOLS_MSG_ALT = (
    "Currently called {tool_calls_count} tools, at least {min_tool_calls} are required. "
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please output the section content immediately starting with "Final Answer:" based on acquired info.'
)

REACT_UNUSED_TOOLS_HINT = "\n💡 You haven't used: {unused_list}. Suggest trying different tools for multi-perspective info."

REACT_FORCE_FINAL_MSG = "Tool call limit reached. Output Final Answer: and generate section content directly."
...
2. Answer directly, avoiding lengthy reasoning/thinking
...
[Available Tools] (Only use when necessary, max 1-2 calls)
{tools_description}
...
class ReportAgent:
        ...
        return {
            ...
            "quick_search": {
                ...
                "parameters": {
                    ...
                    "limit": "Number of return results (optional, default 10)"
                }
            },
            "interview_agents": {
                ...
                "parameters": {
                    ...
                    "max_agents": "Maximum number of interviewed Agents (optional, default 5, max 10)"
                }
            }
        }
    ...
    # Valid tool name set, used for verification during bare JSON fallback parsing
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Format 2: Fallback - LLM outputs bare JSON directly (not wrapped in <tool_call> tag)
        # Try only when Format 1 fails to match to avoid mis-matching JSON in the body
    ...
    def _is_valid_tool_call(self, data: dict) -> bool:
        ...
        if tool_name and tool_name in self.VALID_TOOL_NAMES:
            # Unify key names as name / parameters
            if "tool" in data:
    ...
    def plan_outline(
        ...
        Uses LLM to analyze simulation requirements and plan the directory structure of the report
    ...
    def _generate_section_react(
        ...
        Args:
            ...
            previous_sections: Content of previous sections (to maintain coherence)
        ...
        # Build user prompt - each completed section passed with max 4000 characters
        ...
        for iteration in range(max_iterations):
            ...
            # Check if LLM return is None (API anomaly or content empty)
            if response is None:
                logger.warning(f"Section {section.title} Iteration {iteration + 1}: LLM returned None")
                # If there are iterations left, add message and retry
                if iteration < max_iterations - 1:
                    messages.append({"role": "assistant", "content": "(Empty response)"})
                    messages.append({"role": "user", "content": "Please continue generating content."})
                    continue
                # Last iteration also returns None, break loop and enter force completion
                break
            ...
            if has_final_answer:
                # Insufficient tool calls, refuse and ask for more tools
            ...
            if has_tool_calls:
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM tried to call {len(tool_calls)} tools, executing only first: {call['name']}")
        ...
        # Check if LLM returns None during force completion
        if response is None:
            logger.error(f"Section {section.title} LLM returned None during force completion, using default error message")
